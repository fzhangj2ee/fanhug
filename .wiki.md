# Project Summary
FanHug is a secure payment web application designed to enhance the live sports betting experience. It features a user-friendly interface for registration and login through traditional email/password or OAuth2 social media accounts. The platform integrates with Stripe for secure transactions and The Odds API for real-time betting data, automating bet grading and providing live updates. An admin dashboard facilitates efficient user management. Recent enhancements include a donation page, improved bet slip functionality, a new "Live" page for ongoing games, and a dynamic version number display in the header for easy identification of the deployed version.
...
- **Version Display**:
  - Displays the current version number dynamically in the navbar, positioned right after the FanHug logo, styled subtly for clarity.
...
│   ├── fanhug-logo.png     # Updated colorful heart logo image for the application
...
- **public/**: Holds static assets like the favicon and the new logo.
...
- **src/components/**: UI components, including updated betting components and Navbar with logo and dynamic version display.
...
5. **Deployment**: For deployment, contact MGX support if the publish button is not visible, or consider alternative platforms temporarily. Ensure the `.env` file changes are included in the build and deployment process.
