# Project Summary
FanHug is a secure web application designed to enhance the live sports betting experience through an intuitive user interface. Users can register and log in using email/password or OAuth2 social media accounts. The application integrates with Stripe for secure payment processing and The Odds API for real-time betting data, automating bet grading and providing live updates. An admin dashboard is available for user management, and the platform features a play money display for convenient wallet access. Recent updates include a user messaging system for direct communication with admins and the migration of betting data storage to a Supabase database for improved persistence and security. The branding has been updated to reflect the current logo.
