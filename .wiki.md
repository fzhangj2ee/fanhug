# Project Summary
FanHug is a web application that enhances the live sports prediction experience through an intuitive interface. It allows users to register via email/password or OAuth2 social accounts and integrates with Stripe for secure payments. The application uses The Odds API for real-time prediction data and features automated pick grading, live updates, an admin dashboard for user management, and a play money feature for risk-free exploration. Recently, the application has introduced live predictions, enabling users to make picks on ongoing games while maintaining a focus on responsible gaming.
