# Project Summary
FanHug is a secure web application that enhances the live sports betting experience through an intuitive user interface. Users can register and log in using email/password or OAuth2 social media accounts. The application integrates with Stripe for secure payment processing and The Odds API for real-time betting data, automating bet grading and providing live updates. An admin dashboard is available for user management, and the platform features a play money display for convenient wallet access. Recent updates include a user messaging system for direct communication with admins and improved data persistence through migration to a Supabase database. The latest enhancements include a dedicated messages feature for admins and improved navigation layout.
...
- **User Messaging System**:
  ...
  - Admins can view messages in a Gmail-style table interface, with indicators for read/unread status.
...
- **Navbar Enhancements**:
  - Admin users now see a "Messages" button instead of "Contact Admin," displaying an unread count badge.
  - Navigation links are evenly spaced and center-aligned, preventing the donation link from being squeezed out.
...
- **supabase-schema.sql**: SQL schema for creating the betting and messages tables in Supabase.
