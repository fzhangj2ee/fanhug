# Project Summary
FanHug is a web application that enhances the live sports prediction experience through an intuitive interface. Users can securely register using email/password or OAuth2 social accounts and make payments via Stripe. The application utilizes The Odds API for real-time prediction data, featuring automated pick grading, live updates, and an admin dashboard for user management. FanHug promotes responsible gaming with a play money feature, allowing users to engage in predictions without financial risk, and includes a donation page supporting its mission of restoring integrity in sports.
