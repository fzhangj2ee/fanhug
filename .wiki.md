# Project Summary
FanHug is a web application designed to enhance the live sports prediction experience through an intuitive interface. Users can register using email/password or OAuth2 social accounts and securely make payments via Stripe. The app integrates with The Odds API for real-time prediction data, offering features such as automated pick grading, live updates, and an admin dashboard for comprehensive user management. FanHug promotes responsible gaming through a play money feature, allowing users to make predictions on ongoing games without financial risk. A donation page supports the mission of restoring integrity in sports.
...
- **Pick Time Display and Sorting**:
  - Shows game start times and sorts picks in reverse chronological order.
...
- **Admin Dashboard Fix**:
  - Enhanced logic to load all users' bets for admin view.
  - Added comprehensive logging to troubleshoot issues with user bets not appearing.

- **Betting Context Update**:
  - Fixed a React closure issue that prevented the admin from loading all bets.
  - Added extreme debugging to improve the admin check logic.
