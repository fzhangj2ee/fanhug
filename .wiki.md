# Project Summary
FanHug is a web application designed to enhance the live sports prediction experience with an intuitive interface. It allows users to register using email/password or OAuth2 social accounts, and securely processes payments via Stripe. The app integrates The Odds API for real-time prediction data, featuring automated pick grading, live updates, and an admin dashboard for user management. FanHug promotes responsible gaming through a play money feature, enabling users to make predictions on ongoing games. The donation page supports the mission of restoring integrity in sports.
...
- **Admin Dashboard**:
  ...
  - Displays all user bets, including those placed by regular users.
  - Enhanced debugging added to resolve issues with displaying user bets.
...
- **Wake Up Call Icon Update**:
  - Changed the "Wake Up Call" card icon from a triangle to a yellow bell for better visibility.

- **Admin Dashboard Fix**:
  - Enhanced logic to load all users' bets for admin view, ensuring visibility of all users and their betting statistics.
