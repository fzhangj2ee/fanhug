# Project Summary
FanHug is a secure payment web application designed to enhance the live sports betting experience. It features a user-friendly interface for registration and login via email/password or OAuth2 social media accounts. The platform integrates with Stripe for secure payment processing and The Odds API for real-time betting data, automating bet grading and providing live updates. An admin dashboard facilitates user management. Recent enhancements include a donation page, improved bet slip functionality, a new "Live" page for ongoing games, and a dynamic version number display sourced from the build number. The logo has been restored to its original colorful design to maintain brand consistency, and the current build version has been updated to v176.
...
- **Live Betting**: 
  ...
  - Includes a new "Live" page for ongoing games.
...
- **Donation Page**:
  ...
  - Updated heart icon to a solid red color for better visibility.
...
- **Version Display**:
  - The current version number is dynamically sourced from the build environment variable, allowing for automatic updates with each deployment. The version is currently hardcoded to v176.
...
│   ├── fanhug-logo.png     # Original colorful heart logo image for the application
...
├── vite.config.ts          # Vite configuration with updated build number integration
...
- **public/**: Holds static assets like the favicon and the original logo.
