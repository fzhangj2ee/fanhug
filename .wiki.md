# Project Summary
FanHug is a web application that enhances the live sports betting experience through an intuitive interface. It allows users to register via email/password or OAuth2 social media accounts. The platform integrates with Stripe for secure payments and The Odds API for real-time betting data. FanHug automates bet grading, provides live updates, and features an admin dashboard for user management. A play money option enables users to explore functionalities risk-free, promoting responsible gaming with transparency regarding operational costs.
...
- **Odds API Key Configuration**:
  - Updated instructions for adding the Odds API key in `.env.local`.
...
- **Environment Setup Instructions**:
  - Added guidance for creating a `.env.local` file for Supabase credentials and Odds API key.
...
└── src/lib/sportsApi.ts    # Updated with new Odds API key usage
```
