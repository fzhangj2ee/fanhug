# Project Summary
FanHug is a web application that enhances the live sports prediction experience through an intuitive interface. It allows users to register using email/password or OAuth2 social accounts and make secure payments via Stripe. The app integrates with The Odds API for real-time prediction data, offering features such as automated pick grading, live updates, and an admin dashboard for user management. FanHug promotes responsible gaming with a play money feature, enabling users to make predictions without financial risk. A donation page supports its mission of restoring integrity in sports.
...
- **Simplified Logging in Betting Context**:
  - Streamlined logging for better clarity and reduced complexity.
