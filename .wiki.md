# Project Summary
FanHug is a web application designed to elevate the live sports betting experience with a user-friendly interface. It allows users to register through email/password or OAuth2 social media accounts, integrates with Stripe for secure payments, and utilizes The Odds API for real-time betting data. The platform automates bet grading, provides live updates, and includes an admin dashboard for user management. A play money feature enables users to explore functionalities risk-free, while the project emphasizes responsible gaming and community support, ensuring transparency about operational costs.
...
- **Automatic Bet Grading**:
  - Automatically grades bets every 60 seconds.
...
- **Odds API Key Update**:
  - Ensures continued functionality with updated key.

- **About FanHug Page**:
  - Simplified layout focusing on core information.

- **Bet Placement Balance Check**:
  - Updated to ensure wallet balance is checked before placing bets.
...
└── src/contexts/BettingContext.tsx # Updated betting context with debugging logs
...
- **src/components/**: UI components, including updated betting components and Navbar.
- **src/hooks/**: Custom hooks for accessing authentication context.
- **supabase-schema.sql**: SQL schema for creating the betting and messages tables.
- **.env**: Contains environment variables.
- **.timeline.json**: A hidden system file managed by the MGX platform.
...
5. **Deployment**: For deployment, contact MGX support if needed and ensure environment variables are included in the build process.
