# Project Summary
FanHug is a secure web application designed to enhance the live sports betting experience through an intuitive user interface. Users can register and log in using email/password or OAuth2 social media accounts. The platform integrates with Stripe for secure payment processing and The Odds API for real-time betting data, automating bet grading and providing live updates. An admin dashboard is available for user management, and the application features a play money display for convenient wallet access. The current version is dynamically sourced from the build environment variable. Recent updates have migrated betting data storage from localStorage to a Supabase database for improved data persistence and security.
...
- **Automatic Bet Grading**:
  ...
  - Updates bet statuses from "pending" to "won" or "lost" based on game results and credits winning bets to the user's wallet.
...
- **Data Migration to Supabase**:
  ...
  - Implemented SQL schema for the betting tables in Supabase.
  - Updated BettingContext to interact with Supabase for all betting-related operations.
...
├── supabase-schema.sql     # SQL schema for Supabase betting tables
...
- **supabase-schema.sql**: SQL schema for creating the betting tables in Supabase.
